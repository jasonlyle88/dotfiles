...
If rules within this file conflict, lower-numbered priority wins:
...
- Ask before choices that change indented behavior, API/UX, naming, persistence, auth, dependencies, config, or compatibility that affects callers/users.
...
- Follow language best practices for anything not covered in this section.
- Code should be written in small, concise methods allowing for:
    - Following the unix philosophy of "do one thing and do it well"
    - Following the DRY principle (do not repeat yourself)
- Follow the project formatter; if no convention exists, default to 4 spaces for indentation.
- Do not mix broad formatting-only changes with functional edits. Formatting is acceptable only in the lines or sections already touched for the requested change.
- Ensure code is commented in a useful manner. A call to a function named `get_api_key` does not need a comment saying `get the API key`, because this is already obvious. Comments should be helpful, not there for the sake of having a comment for every line of code.
